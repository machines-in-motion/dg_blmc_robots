...
if __name__ == "__main__":
    ...
    folder = os.path.join(expanduser("~"),"fgrimminger_paper_log",
      "stiffness_data_compain_v2_plot")

    file_names = [
      os.path.join(folder, "2019-07-04_16-03-13", "2019-07-04_16-03-13.npz"),
      os.path.join(folder, "2019-07-04_16-04-15", "2019-07-04_16-04-15.npz"),
      os.path.join(folder, "2019-07-04_16-05-17", "2019-07-04_16-05-17.npz"),
      os.path.join(folder, "2019-07-04_16-05-54", "2019-07-04_16-05-54.npz"),
      os.path.join(folder, "2019-07-04_16-06-36", "2019-07-04_16-06-36.npz"),
      os.path.join(folder, "2019-07-04_16-07-42", "2019-07-04_16-07-42.npz"),
      os.path.join(folder, "2019-07-04_16-08-19", "2019-07-04_16-08-19.npz"),
      os.path.join(folder, "2019-07-04_16-09-06", "2019-07-04_16-09-06.npz"),
      os.path.join(folder, "2019-07-04_16-09-53", "2019-07-04_16-09-53.npz"),
      os.path.join(folder, "2019-07-04_16-11-57", "2019-07-04_16-11-57.npz"),
      os.path.join(folder, "2019-07-04_16-12-42", "2019-07-04_16-12-42.npz"),
      os.path.join(folder, "2019-07-04_16-15-52", "2019-07-04_16-15-52.npz"),
      os.path.join(folder, "2019-07-04_16-16-51", "2019-07-04_16-16-51.npz"),
      os.path.join(folder, "2019-07-04_16-17-43", "2019-07-04_16-17-43.npz"),
      os.path.join(folder, "2019-07-04_16-20-09", "2019-07-04_16-20-09.npz"),
      os.path.join(folder, "2019-07-04_16-21-00", "2019-07-04_16-21-00.npz"),
      os.path.join(folder, "2019-07-04_16-24-16", "2019-07-04_16-24-16.npz"),
      os.path.join(folder, "2019-07-04_16-25-06", "2019-07-04_16-25-06.npz"),
    ]
    nb_files = len(file_names)

    list_labels = []
    for i in range(nb_files):
        list_labels.append("K="+str((i+1)*20)+"N/m")
    ...
    #
    fig = plt.figure("quasistatic_experiment")
    ...
    plt.plot(dx, Fz_max, label = "Max force (N)", color="black", linewidth=3.0)
    ...
    for i in range(nb_files):
        ...
        # plt.plot(dx, f, label = "Theoretical stiffness, k=" + str(k), color="forestgreen")        
        
        # Compute and plot actual stiffness
        x = list_pos_error_height_sensor[i].copy()
        y = list_Fz[i].copy()
        y = y[ np.all( [0 < x, x < 0.07], axis=0) ]
        x = x[ np.all( [0 < x, x < 0.07], axis=0) ]
        poly = polyfit(x, y, 1)
        poly['polynomial'][0]
        dx = np.linspace(np.min(list_pos_error_height_sensor[i]), np.max(list_pos_error_height_sensor[i]), num=2000, endpoint=True)
        f = poly['polynomial'][0] * dx + poly['polynomial'][1]
        actual_stiffeness_plot = plt.plot(dx,f, label = "", linewidth=1.0, linestyle="--")

        # plot mean data
        fz = np.array([])
        deltaz = np.linspace(np.min(list_pos_error_height_sensor[i]), np.max(list_pos_error_height_sensor[i]), num=100, endpoint=True)
        for j in range(deltaz.shape[0]-1):
            fz = np.append( fz,  np.mean(   list_Fz_cutoff[i][ np.all( [deltaz[j] < list_pos_error_height_sensor[i], list_pos_error_height_sensor[i] < deltaz[j+1]], axis=0) ]  ) )
        plt.plot(deltaz[:-1], fz,
                 label= list_labels[i] + "K_m={:.2f}N/m".format(poly['polynomial'][0]),
                 color=actual_stiffeness_plot[0].get_color(),
                 linestyle="-",
                 linewidth=3.0)

        # plot actual data
        # plt.plot(list_pos_error_height_sensor_cutoff[i], list_Fz_cutoff[i],
        #          label="Fz(dZ)",
        #          color="dodgerblue",
        #          linestyle=actual_stiffeness_plot[0].get_linestyle())


        fig.legend(prop={'size': 18})
        plt.grid()
        plt.xlabel("Vertical Foot Displacement (dZ) [m]")
