...
class SoloBaseRobot(Robot):
    ...
    def __init__(
        ...
        pybullet.resetDebugVisualizerCamera(1.2, 50, -35, (0.0, 0.0, 0.0))
        ...
        pybullet.setPhysicsEngineParameter(fixedTimeStep=1.0 / 1000.0, numSubSteps=1)
        ...
        self.wrapper = PinBulletWrapper(
            self.robotId, self.pin_robot, self.joint_names, self.end_effector_names
        )
        ...
        self.device.slider_positions.value = np.array(4 * [0.0])
    ...
    def start_video_recording(self):
        if self.record_video:
            pybullet.startStateLogging(
                pybullet.STATE_LOGGING_VIDEO_MP4, "/tmp/bullet_quadruped.mp4"
            )
    ...
    def sim2signal_(self):
        ...
        self.signal_base_vel_world_.sout.value = self.wrapper.get_base_velocity_world()

        device.slider_positions.value = np.array([
            pybullet.readUserDebugParameter(self.slider_a),
            pybullet.readUserDebugParameter(self.slider_b),
            pybullet.readUserDebugParameter(self.slider_c),
            pybullet.readUserDebugParameter(self.slider_d),
        ])
    ...
    def run(self, steps=1, delay=0.0, plot=False):
        ...
        for i in range(steps):
            ...
            self.wrapper.send_joint_command(
    ...
    def set_gravity(self, vec):
        ...
        the vertical axis."""
        pybullet.setGravity(vec[0], vec[1], vec[2])
    ...
    def print_physics_params(self):
        ...
        for ji in range(num_joints):
            (
                ...
                contact_damping,
                contact_stiffness,
            ) = pybullet.getDynamicsInfo(bodyUniqueId=self.robotId, linkIndex=ji)
